...
// resulting public URL to rooms.image_url. Storage path: {hotelId}/{roomId}/{filename}.
// Returns the public URL. (Bucket + image_url column are created by a separate migration.)
export async function uploadRoomImage(roomId, hotelId, file) {
  const safeName = (file?.name || "photo").replace(/[^\w.\-]+/g, "_");
  const path = `${hotelId}/${roomId}/${safeName}`;
