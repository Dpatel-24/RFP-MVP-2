...
// Timing constants — the DB is authoritative for requests.expires_at (set by
// column default on insert and by a trigger when a counter is sent). These
// mirror those server values purely to drive the display countdown, so they
// must stay in sync with the intervals in the expiry migration.
export const TIMER_SECONDS = 60;  // guest bid -> hotel response
export const COUNTER_TIMER = 120; // hotel counter -> guest response
export const TAX_RATE = 0.14;     // est. taxes shown to guests at confirm/live

// Single source for response-window copy so it can't drift from TIMER_SECONDS.
export const RESPONSE_LABEL = TIMER_SECONDS >= 60 && TIMER_SECONDS % 60 === 0
  ? `${TIMER_SECONDS / 60} minute${TIMER_SECONDS === 60 ? "" : "s"}`
...
function mapHotel(h) {
  ...
  return {
    ...
    // Informational only — never enters revenue, analytics, or bid math.
    // The deposit is a hold the hotel collects at check-in, not LastKey.
    depositAmount: h.deposit_amount != null ? Number(h.deposit_amount) : 0,
    phone: h.phone || null,
...
// Live "stays" derived from actual transaction data: the number of this guest's
// requests that reached accepted/handled. Avoids relying on guest_profiles
// .stays_count, which is a stored field that was never being incremented.
export async function getGuestStats(guestId) {
  const { count, error } = await supabase
    .from("requests")
    .select("id", { count: "exact", head: true })
    .eq("guest_id", guestId)
    .in("status", ["accepted", "handled"]);
  if (error) { console.error(error); return { stays: 0 }; }
  return { stays: count || 0 };
}

export async function getGuestProfile(userId) {
  const { data } = await supabase
    .from("guest_profiles")
    .select("id,name,first_name,last_name,phone,rating,created_at")
    .eq("id", userId)
    .maybeSingle();
  if (!data) return null;
  const { stays } = await getGuestStats(userId); // live count, not the stale stays_count field
  return {
    id: data.id,
...
// Guest trust signals for the hotel-side bid card: rating + GLOBAL stays.
// Uses the guest_card_stats() SECURITY DEFINER RPC, which counts accepted/handled
// requests across ALL hotels (the hotel's own RLS only sees its own rows) and is
// gated so a hotel can only read stats for a guest who bid with them.
export async function fetchGuestProfile(guestId) {
  if (!guestId) return null;
  const { data, error } = await supabase.rpc("guest_card_stats", { g_id: guestId });
  if (error) { console.error(error); return { id: guestId, rating: 0, stays: 0, verified: false }; }
  const row = Array.isArray(data) ? data[0] : data;
  return {
    id: guestId,
    rating: Number(row?.rating || 0),
    stays: Number(row?.stays || 0),
    verified: false, // no verified column yet — badge stays hidden until one exists
  };
}
...
export async function getHotelsWithRooms() {
  const { data, error } = await supabase
    .from("hotels")
    .select(
      "id,name,location,city,tagline,rating,review_count,google_place_id,deposit_amount,phone," + // google_place_id: [GOOGLE-REVIEWS TEST]; deposit/phone: informational display only
...
export async function submitBid({ hotelId, roomId, guestId, amount }) {
  const { data, error } = await supabase
    ...
    .insert({
      ...
      // expires_at intentionally omitted — the DB column default stamps it
      // server-side so the window can't be shifted by a skewed browser clock.
    })
    .select(guestEmbed)
    .single();
  if (error) throw error;
...
// NOTE: cancelRequest() was removed. Guests can no longer cancel — the response
// window is 60s, too short to be worth a cancel path. It was also non-functional:
// RLS policy requests_guest_respond_counter only permits a guest UPDATE when
// status='countered' and only to 'accepted'/'declined', so writing 'cancelled'
// silently matched zero rows. The 'cancelled' status remains valid in the CHECK
// constraint and is still rendered by Badge for historical rows.
...
// ── Saved hotels (guest favourites) ──────────────────────────────────────────
// Assumes a saved_hotels table (id, guest_id, hotel_id, created_at) with a UNIQUE
// (guest_id, hotel_id) constraint for the upsert. Migration is run separately.
// SECURITY REMINDER: saved_hotels MUST have RLS so a guest can only read/write
// their own rows, e.g. USING/WITH CHECK (auth.uid() = guest_id) for
// select/insert/delete. Run the Supabase security advisor after the table's
// migration to confirm RLS is enabled and the policies are present.
export async function saveHotel(guestId, hotelId) {
  const { error } = await supabase
    .from("saved_hotels")
    .upsert({ guest_id: guestId, hotel_id: hotelId }, { onConflict: "guest_id,hotel_id" });
  if (error) throw error;
}
export async function unsaveHotel(guestId, hotelId) {
  const { error } = await supabase
    .from("saved_hotels")
    .delete()
    .eq("guest_id", guestId)
...
export async function getSavedHotels(guestId) {
  const { data, error } = await supabase
    .from("saved_hotels")
    .select("hotel_id")
    .eq("guest_id", guestId);
...
export async function getOwnerHotel(userId) {
  const { data, error } = await supabase
    .from("hotels")
    .select("id,name,location,city,tagline,rating,review_count,deposit_amount,phone")
...
// Informational hotel fields (deposit hold + contact phone). Display-only:
// callers must never feed these into revenue, analytics, or bid amounts.
export async function updateHotelInfo(hotelId, { deposit_amount, phone }) {
  const { error } = await supabase
    .from("hotels")
    .update({ deposit_amount, phone })
    .eq("id", hotelId);
  if (error) throw error;
}
...
export async function hotelCounter(id, amount) {
  const { error } = await supabase
    ...
    .update({
      ...
      // expires_at intentionally omitted — trg_stamp_counter_expiry restamps
      // the window server-side (COUNTER_TIMER) on the transition to countered.
    })
    .eq("id", id);
  if (error) throw error;
}
