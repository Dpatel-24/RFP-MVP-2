...
// Lean guest profile for the hotel-side bid card: rating + (live) stays + verified.
// Rating comes from guest_profiles (RLS guest_visible_to_hotel lets a hotel read
// the profile of any guest who bid with them). Stays uses getGuestStats — the
// same accepted/handled derivation as the guest side.
// NOTE: under the hotel's RLS (requests_hotel_read) getGuestStats counts only the
// guest's accepted stays AT THIS HOTEL. A global cross-hotel count needs a
// SECURITY DEFINER stats RPC, deferred as a future improvement.
export async function fetchGuestProfile(guestId) {
  ...
  const { data } = await supabase
    ...
    .eq("id", guestId)
    .maybeSingle();
  const { stays } = await getGuestStats(guestId);
  return {
    id: guestId,
    rating: Number(data?.rating || 0),
    stays,
